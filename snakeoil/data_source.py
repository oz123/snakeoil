...
* py3k concerns (bytes versus text file handles)

Note that :py:class:`text_data_source` and :py:class:`bytes_data_source` are
separate classes doing necessary conversion steps for bytes/text requests.  Use
the appropriate one- it'll save yourself a headache when dealing with bytes vs
text.
